...
- `ttyS1` 登录口当前使用 `921600 8N1`、无硬件流控、`xterm-256color`、`COLORTERM=truecolor`、固定 `32x64` 和 `--autologin root`；终端环境通过精确匹配 `/dev/ttyS1` 的 profile 脚本设置，不影响 ttyS0、SSH、ADB 或其他终端。代价是 `llm-sys.service` 已 mask/inactive，M5-Bus StackFlow/JSON 主控通信不可用。
...
- 2026-06-12 追加观察：Android 官方截至 2026-06-04 的稳定 Platform Tools 仍为 `37.0.0`。ttyS1 已完成 Tab5 终端适配：`agetty` 类型改为 `xterm-256color`，ttyS1 登录 shell 设置 `COLORTERM=truecolor` 和 `32x64`。当前会话和设备重启后的真实 ttyS1 shell 均返回 `TERM=xterm-256color`、`COLORTERM=truecolor`、`stty size=32 64`、`tput colors=256`，`htop` 初始化测试通过。
