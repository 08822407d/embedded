...
void UNIT_JOYC::writeBytes(uint8_t addr,
		uint8_t reg, uint8_t *buffer, uint8_t length) {
...
void UNIT_JOYC::readBytes(uint8_t addr, uint8_t reg,
...
bool UNIT_JOYC::begin(TwoWire *wire, uint8_t addr,
		uint8_t sda, uint8_t scl, uint32_t speed) {
	_wire  = wire;
	_addr  = addr;
	_sda   = sda;
	_scl   = scl;
