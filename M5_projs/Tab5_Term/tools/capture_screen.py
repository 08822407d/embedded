#!/usr/bin/env python3
"""Capture the Tab5 LVGL screen over the private USB serial debug protocol."""
...
def main() -> int:
    ...
    with ser:
        ...
        if pixel_format != "rgb565le" or byte_count != width * height * 2:
            raise ValueError(f"unsupported capture format: {pixel_format}, bytes={byte_count}")
    ...
    if actual_crc != expected_crc:
        raise ValueError(f"CRC mismatch: device={expected_crc:08X}, host={actual_crc:08X}")
