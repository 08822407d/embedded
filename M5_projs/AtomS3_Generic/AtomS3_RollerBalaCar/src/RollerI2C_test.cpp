...
double angleKp = 300.0, angleKi = 0.5, angleKd = 50.0;
...
void setup()
{
	...
	RollerI2C_1.begin(&Wire, 0x64, 2, 1, 400000);
	RollerI2C_2.begin(&Wire, 0x65, 2, 1, 400000);
...
void normal() {
    ...
    // 2. 获取电机当前转速（>0 或 <0）
	double currentSpeed = RollerI2C_1.getSpeed();
	RollerI2C_1.setOutput(1);
	...
	RollerI2C_1.setSpeed(finalSpeed);
	RollerI2C_1.setOutput(1);
}