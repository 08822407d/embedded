...
> 核心矛盾:**被忽略 = 无 git 备份;CubeMX / MC Workbench 重生成 = 覆盖生成文件。** 两者叠加 → 手动改动可能**既没有 git 备份、又被重生成抹掉**。本文件说清忽略了什么、改动时怎么保、覆盖后怎么恢复。
...
| **`/*_foc/`**(当前为 `GM16020-06_foc/`) | **MC Workbench + 内置 CubeMX 生成的正式 FOC 工程树**(`Drivers/`、MCSDK、`Src/Inc`、`.ioc`、CubeIDE 工程和日志);完成 Debug 构建后当前约 1638 个文件/162 MB | **高 —— 正式应用代码以后很可能改 `Src/Inc`,见 §2/§3** | **同名 `.stwb6` 种子 + MC Workbench 6.4.2 重生成**;命名约定为生成目录以 `_foc` 结尾 |
...
**保留在库的"种子/锚点"(不忽略):** `*.stwb6`(MC Workbench 工程,重生成的唯一输入;例如 `GM16020-06_foc.stwb6`)、`*.ioc` 冒烟种子、`cubemx_generate*.txt`、`build_win.ps1`、本 `docs/`。

## 2. 风险模型(动手前必读)
- 生成工程(如 `GM16020-06_profile/`、`GM16020-06_foc/`)**不在 git 里** → 你对它的手改**没有版本历史**,误删/覆盖后无法 `git restore`。
- 用 MC Workbench / CubeMX **重生成**该工程 → **覆盖生成文件**。唯一天然幸存的是 CubeMX 的 **`USER CODE BEGIN … USER CODE END` 之间**的内容;此标记之外的改动、以及被整体重写的文件,**一律丢失**。
- 正式 FOC 工程尤其要注意:`Src/Inc` 虽然体积不大,但仍位于被忽略的生成树中;不能因为它们是 C 源码就假定 git 会备份。
...
- **B1(推荐)取反解除忽略**:在 `.gitignore` 里把**该具体文件**放行入库。注意父目录被忽略时需**逐级放行**。例如正式 FOC 工程必须保留 `Src/main.c` 的非 USER CODE 改动时:
  ```gitignore
  # 放行 GM16020-06_foc/Src/main.c
  !GM16020-06_foc/
  GM16020-06_foc/*
  !GM16020-06_foc/Src/
  GM16020-06_foc/Src/*
  !GM16020-06_foc/Src/main.c
  ```
  （只放行确需备份的少数文件,别把整棵大树又放回来。）
- **B2 备份副本**:把改过的文件复制到**已跟踪目录**(如 `profiler_overrides/` 或 `foc_overrides/<工程名>/`,镜像相对路径),入库;并在 §4 台账记明"源路径→备份路径"。重生成后按台账拷回/合并。
...
## 4. 手动改动台账(所有被忽略的 MC Workbench 生成工程)
> 覆盖 `GM16020-06_profile/`、`GM16020-06_foc/` 及后续 `*_foc/`。空表 = 目前**未对这些生成工程做任何手改**(它们是 MC Workbench/CubeMX 原样生成)。一旦手改,**立即在此加行**。

| 日期 | 生成工程 | 文件(相对生成工程根) | 改动摘要 | 原因 | 在 USER CODE 内? | 备份方式(A/B1/B2) | 重生成后状态 |
|---|---|---|---|---|---|---|---|
| — | — | — | (暂无) | — | — | — | — |
...
- 新建正式 FOC 工程时,生成目录统一命名为 `<名称>_foc/`,对应种子命名为 `<名称>_foc.stwb6`;若不遵守该命名,必须另加精确忽略规则。
