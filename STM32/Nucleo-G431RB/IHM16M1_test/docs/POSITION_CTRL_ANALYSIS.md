...
- **做什么**:**保持现在的霍尔速度 FOC 固件**,在应用/固件层加"数圈"逻辑:
  - ⚠️ **2026-07-16 源码预检修正(F-39)**:原设想的 `MC_REG_CURRENT_POSITION` 在本构建**零实现**(仅头文件样板,`sync_registers.c` 无 case),**不可用**;本构建 MCP 唯一角度观测量 = `MC_REG_HALL_EL_ANGLE`(s16 **电角度**,`sync_registers.c:859` `SPD_GetElAngle(HALL_M1)`,每电气圈回绕;4 对极 → 4 电气圈 = 1 机械圈)→ **PC 侧对电角度解回绕计圈**;固件侧备选 `SPD_GetMecAngle`(基类机械角,Hall 组件累计行为待核);
...
## 4. 结论(已拍板)
**2026-07-16 用户选定 B 方案** → 落为 DECISIONS **#007**、REQUIREMENTS **§6**(需求/验收)、[`CODEX_TASK_revcount_poc.md`](CODEX_TASK_revcount_poc.md)(实施任务书)。
用户理由:目标应用配**大减速比减速箱**,电机侧整圈级误差被稀释,预计低于机械工艺误差;应用转速远高于 ~500rpm 下限。
A(加编码器)保留为日后需要精确角度/位置保持时的升级路径;C 否决。
`MC_REG_CURRENT_POSITION` 预检已完成:零实现、不可用(F-39),实施改用 `HALL_EL_ANGLE` 解回绕(见 §2 路线 B 修正)。
