...
## #007 — 2026-07-16 — 位置控制走 B 方案:应用层"数圈"粗控(不加编码器、不用官方位置模式)
- **背景**:官方 MCSDK 位置模式要求正交编码器为主传感器、霍尔被明确禁止(F-38);本电机 GM16020-06 无编码器轴;用户目标精度 = **整圈级**。三选项见 [`POSITION_CTRL_ANALYSIS.md`](POSITION_CTRL_ANALYSIS.md) §2。
- **决定**:选 **B** —— 保持现有霍尔速度 FOC 固件(F-36 ELF 不动),应用层数机械圈、到目标圈数减速停。**Phase-1 PoC 在 PC 侧实现(零固件改动)**:经 MCP 读取 `MC_REG_HALL_EL_ANGLE` 解回绕计圈 + `SPEED_RAMP`/Stop 控制;固件侧(Phase-2)仅在应用需要脱机运行时再做。
- **用户理由(风险接受依据,2026-07-16)**:目标应用配**大减速比减速箱**,电机侧停位过冲/整圈误差经减速比稀释后,量级预计低于整套机械硬件的工艺误差;且应用转速远高于 ~500rpm 低速下限,低速区弱点不构成障碍。
- **实现前提修正**:原设想的 `MC_REG_CURRENT_POSITION` 经源码预检为**零实现**(F-39),数圈只能用电角度解回绕(PC 侧)或 `SPD_GetMecAngle`(固件侧,Hall 组件机械角累计行为待核)。
- **否决**:A(加编码器)——精度冗余且 16mm 电机/1.5mm 轴机械加装难,留作日后需要精确角度/位置保持时的升级路径;C(魔改霍尔进官方位置环)——手改生成树维护成本高、脱离官方支持、仍受 60° 分辨率与低速限制。
- **配套**:REQUIREMENTS §6(需求与验收)、[`CODEX_TASK_revcount_poc.md`](CODEX_TASK_revcount_poc.md)(实施)。
