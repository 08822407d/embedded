...
| F-12 | **Windows 侧 ST 工具已就位**:CubeCLT **1.18.0**(`C:\Program Files\STMicroelectronics\STM32CubeCLT_1.18.0`, `STM32_Programmer_CLI` **v2.19.0**, `arm-none-eabi-gcc` **13.3.1**);CubeIDE **1.19.0**(含 GNU Make **4.4.1_st_20231030-1220** 与 bundled CubeProgrammer **2.20.0**);CubeMX **6.17.0-RC5**(`C:\Program Files\STMicroelectronics\STM32Cube\STM32CubeMX`);standalone CubeProgrammer **2.18.0**;G4 FW **V1.6.1/V1.6.2** 已安装;X-CUBE-MCSDK **6.4.1** 已安装(`C:\Program Files (x86)\STMicroelectronics\MC_SDK_6.4.1`, MC Workbench `STMCWB.exe` v6.4.1, MotorPilot 存在) | Windows codex 只读盘点(`Get-Command`/`Get-ChildItem`/`--version`/file version) | 2026-07-03 |
