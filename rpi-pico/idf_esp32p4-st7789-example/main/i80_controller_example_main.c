...
static void
example_increase_lvgl_tick(void *arg) {
	/* Tell LVGL how many milliseconds has elapsed */
	lv_tick_inc(EXAMPLE_LVGL_TICK_PERIOD_MS);
}

static void
example_lvgl_port_task(void *arg) {
...
void example_init_spi_bus(esp_lcd_panel_io_handle_t *io_handle)
{
	ESP_LOGI(TAG, "Initialize SPI bus");
	spi_bus_config_t buscfg = {
		.sclk_io_num = EXAMPLE_PIN_NUM_PCLK,
		.mosi_io_num = CONFIG_EXAMPLE_PIN_NUM_DATA0,
		.miso_io_num = -1,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1,
...
void app_main(void)
{
	...
	example_init_spi_bus(&io_handle);
